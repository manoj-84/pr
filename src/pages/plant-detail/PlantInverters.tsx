...
export default function PlantInverters({ plant }: PlantInvertersProps) {
  ...
  const plantInverters = inverters.filter(i => i.plantId === plant.id);

  const onlineInverters = plantInverters.filter(i => i.status === "online").length;
  const warningInverters = plantInverters.filter(i => i.status === "warning").length;
  const offlineInverters = plantInverters.filter(i => i.status === "offline").length;
  const totalOutput = plantInverters.reduce((sum, i) => sum + i.output, 0);

  const selectedInverterData =
    selectedInverter === "all" ? null : plantInverters.find(i => i.id === selectedInverter);
              ...
              >
                <CardContent className="p-4">
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      ...
                      <p className="text-xs opacity-80 mt-0.5">
                        {inverter.statusDetail || inverter.status}
                      </p>
                    </div>
                  ...
                  <div className="space-y-1">
                    ...
                    {inverter.temperature !== undefined && (
                      <div className="flex justify-between text-xs">
                        <span>Temp:</span>
        ...
        <Card className="bg-card border-border">
          <CardHeader>
            <CardTitle className="text-base">
              Detailed Diagnostics: {selectedInverterData.name}
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              <div>
                ...
                <p className="text-lg font-bold text-foreground">
                  {(selectedInverterData.output * 1.03).toFixed(1)} kW
                </p>
              ...
              <div>
                ...
                <p className="text-lg font-bold text-foreground">
                  {(
                    (selectedInverterData.output /
                      (selectedInverterData.output * 1.03)) *
                    100
                  ).toFixed(1)}
                  %
                </p>
